...
class My_booking extends CI_Controller {
	...
	{	
		if ($this->config->item("api_key") === base64_decode($this->input->post('api_key', TRUE)))
		{
			...
			if ($this->form_validation->run()) {
				...
				$latest_booking = $this->general->get_list_view('car_booking', 'user_id', $data['account']->id, $book_select = NULL, 'booking_id', 'desc', NULL, NULL);
				if ($latest_booking) {
					// Combine all these elements for display
					$route_select = '`route_name_en`, `route_name_bn`';
					$schedule_select = '`route_id`, `start_time`';
					...
					$response["success"] = 1;
					$response["my_booking"] = $data['latest_booking'];
					echo json_encode($response);
				}
				else
				{
					$response["success"] = 0;
					$response["message"] = "You have no any booking";
					echo json_encode($response);
				}
			}
			else
	...
	function search()
	{
		...
		$user_id = array('user_id'=>$data['account']->id);
	...
	function booking_cancelled($booking_id)
	 {
	  ...
	  if (!$this->check_my_booking($booking_id, $this->session->userdata('account_id'))) {
	   $this->session->set_flashdata('message_error', 'You can not modify another user booking');
	   redirect(base_url().'car/my_booking');
	  }

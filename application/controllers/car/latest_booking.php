...
class Latest_booking extends CI_Controller {
	...
	function latest_schedule_booking()
	{
		// Enable SSL?
		maintain_ssl($this->config->item("ssl_enabled"));
		// Redirect unauthenticated users to signin page
		...
		$data['title'] = 'latest Schedule Booking';	
...
ORDER BY car_sdrt_schedule.schedule_date DESC";
		...
		//$count_members = count($all_members);
		$config = array();
		$config['base_url'] = base_url().'car/latest_booking/latest_schedule_booking/';
		$config['total_rows'] = $this->general->number_of_total_rows_in_a_table('car_schedule_booking');
		$config['num_links'] = 3;
		$config['per_page'] = $this->config->item("pagination_perpage");
		$config['uri_segment'] = 4;
		...
		$this->load->view('car/latest_schedule_booking', isset($data) ? $data : NULL);	
	}
	...
	//////////////////////////////////////////////////////
	function booking_schedule_processing($booking_id)
	{
		// Enable SSL?
		maintain_ssl($this->config->item("ssl_enabled"));
		// Redirect unauthenticated users to signin page
		...
		$table_data = array('booking_status'=>3);
		$this->general->update_table('car_schedule_booking', $table_data,'sbooking_id', $booking_id);
		$this->session->set_flashdata('message_success', lang('success_update'));
		redirect('car/latest_booking/latest_schedule_booking');
	}
	
	function booking_schedule_accepted($booking_id)
	{
		...
		$table_data = array('booking_status'=>0);
		$this->general->update_table('car_schedule_booking', $table_data,'sbooking_id', $booking_id);
		$this->session->set_flashdata('message_success', lang('success_update'));
		redirect('car/latest_booking/latest_schedule_booking/');
	}
	
	function booking_schedule_availed($booking_id)
	{
		// Enable SSL?
		maintain_ssl($this->config->item("ssl_enabled"));
		// Redirect unauthenticated users to signin page
		...
		$table_data = array('booking_status'=>1);
		$this->general->update_table('car_schedule_booking', $table_data,'sbooking_id', $booking_id);
		$this->session->set_flashdata('message_success', lang('success_update'));
		redirect('car/latest_booking/latest_schedule_booking/');
	}
	
	function booking_schedule_cancelled($booking_id)
	{
		// Enable SSL?
		maintain_ssl($this->config->item("ssl_enabled"));
		// Redirect unauthenticated users to signin page
	...
	function delete_schedule_booking($booking_id){
		...
		if($this->general->delete_from_table('car_schedule_booking', 'sbooking_id', $booking_id)):
		$this->general->delete_from_table('car_sdrt_cost', 'sbooking_id', $booking_id);
		$this->session->set_flashdata('message_success', 'Your data successfully deleted');
		endif;
		redirect('car/latest_booking/latest_schedule_booking');	
	}

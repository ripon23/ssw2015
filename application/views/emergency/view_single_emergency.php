...
<?php echo $this->load->view('header'); ?>
    ...
    <table class="table table-bordered">
    	<tr>
           ...
           <td><?php echo $emergency_info->total_travel_distance; ?></td>
        </tr> 
        <tr>
           <td>Total travel time</td>
           <td><?=$emergency_info->total_travel_time?></td>
        </tr>
        <tr>
           <td>Total estimated bill</td>
           <td><?=$emergency_info->total_bill?></td>
        </tr> 
        <tr>
           <td>Note</td>
           <td><?=$emergency_info->note?></td>
        </tr> 
    </table>           
    </div>            
    ...
    <table class="table table-striped">    	              
        <tr>
        	<td>Last Update: </td>
            <td><?=$emergency_info->last_edit_date?> </td>
        </tr>
        <tr>
        	<td>Entry/Update user: </td>
            <td><?=$emergency_info->edit_user_id?> </td>
        </tr>
        </table> 
        
        <?php
        if($this->authorization->is_permitted('edit_emergency_services'))
        {
        ?>
        <a href="<?php echo base_url().'emergency/emergency/edit_single_emergency/'.$emergency_info->reg_for_service_id ;?>" class="btn btn-block btn-large btn-warning"><?=lang('website_edit')?></a>
        <?php
        }
        ?>
    </div><!-- /end span3 -->

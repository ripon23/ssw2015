...
var booking_id=document.getElementById('services_booking_id_'+numeric).value;
	var booking_for_service_id=document.getElementById('booking_for_service_id_'+numeric).value;
	...
	var dataString='booking_for_service_id='+booking_for_service_id+'&booking_id='+ booking_id + '&reg_services_point=' + reg_services_point + '&reg_services=' + reg_services + '&reg_services_package=' + reg_services_package +'&services_date='+services_date+'&statusvalue='+statusvalue;	
	$.ajax
		({
			type: "POST",
			url: "booking/booking/update_services",
			data: dataString,
			cache: false,
			success: function(msg)
			{
			alert("Update "+msg+" for: "+booking_id);
			location.reload(); 			
			}
		});	
...
<?php echo $this->load->view('header'); ?>
    ...
    <div class="panel-heading"><img src="<?php echo base_url().RES_DIR; ?>/img/services-icons/registration_48.png" width="48" height="41"> View Booking info.</div>
    ...
    <div class="span3">
    	...
    	<tr>
            ...
            <select name="reg_status" id="reg_status" class="selectpicker span1.5" data-style="btn">
                <option value="0" <?php if($single_booking->status==0) echo 'selected="selected"'; ?> data-content="<span class='label label-warning'>Unregistered</span>">Unregistered</option>                
                
                <option value="2" <?php if($single_booking->status==2) echo 'selected="selected"'; ?> data-content="<span class='label label-important'>Deleted</span>">Deleted</option>
            </select>
...
$(function(){
            ...
            <td>
            <input type="hidden" name="booking_for_service_id_<?=$i?>" id="booking_for_service_id_<?=$i?>" value="<?php echo $services->booking_for_service_id;?>" type="text" />	            
            <input type="hidden" name="services_booking_id_<?=$i?>" id="services_booking_id_<?=$i?>" value="<?php echo $services->booking_id;?>" type="text" />			

...
<?php echo $this->load->view('header'); ?>
    ...
    <?php echo form_open('learning/learning/search_learning_list') ?>
...
<table class="table table-bordered table-striped">
			...
			?>
            <?php 
			if( !empty($all_learning) ) {
			foreach ($all_learning as $learning) : 
			?>
            <tr id="row_<?=$i?>">
				<td><?=$i?></td>
                <td><?php echo $learning->reg_for_service_id; ?></td>
                <td><?php echo $learning->registration_no; ?></td>
                <td><?php 
				$reg_info= $this->registration_model->get_all_registration_info_by_id($learning->registration_no); 
				echo "<a href=".base_url().'registration/registration/view_single_registration/'.$learning->registration_no.">".$reg_info->first_name." ".$reg_info->middle_name." ".$reg_info->last_name."</a>";
				
				?>
</td>
                <td><?php if($learning->services_point_id) echo $this->ref_site_model->get_site_name_by_sp_id($learning->services_point_id); ?></td>
                <td><?php if($learning->services_point_id) echo $this->ref_site_model->get_site_name_by_id($learning->services_point_id); ?></td>
                <td><?php if($learning->services_package_id) echo $this->ref_services_model->get_package_name_by_id($learning->services_package_id); ?></td>
                <td><?php echo $learning->services_date; ?></td>
                <td>
				<?php 
				if($learning->services_status==0)
				{
				echo '<span class="label label-warning">Pending</span>';				
				}
			else if	($learning->services_status==1)
				{
				echo '<span class="label label-info">Process</span>';
				}
			else if	($learning->services_status==2)
				{
				echo '<span class="label label-success">Taken</span>';
				}
			else if	($learning->services_status==3)
				{
				echo '<span class="label label-important">Cancel</span>';
				}
			else if	($learning->services_status==4)
				{
                ...
                <?php //if ($this->authorization->is_permitted('add_learning')) : ?> 
                <td>
                <?php 
				if($learning->services_status==2)
				{
				if ($this->authorization->is_permitted('view_information_services')) : 		
				?>                
                <a href="<?php echo base_url().'learning/learning/view_single_learning/'.$learning->reg_for_service_id;?>" class="btn btn-small btn-success"><?=lang('website_view')?></a>
                <?php
				endif; 
				if ($this->authorization->is_permitted('edit_information_services')) : 	
				?>
                <a href="<?php echo base_url().'learning/learning/edit_single_learning/'.$learning->reg_for_service_id;?>" class="btn btn-small btn-warning"><?=lang('website_edit')?></a>
                <?php
				endif; 
				}
				else
				...
				?>
                <a href="<?php echo base_url().'learning/learning/add_learning/'.$learning->reg_for_service_id;?>" class="btn btn-small btn-info"><?=lang('website_add')?></a>
                <?php
				endif; 
				}
				?>

...
<?php echo $this->load->view('header'); ?>
    ...
    <?php echo form_open('college_bus/college_bus/search_college_bus_list') ?>
...
<th><?=lang('site')?></th>
                ...
                <th><?=lang('status')?></th>               
                <?php if ($this->authorization->is_permitted('add_college_bus_services')) : ?> 
			...
			?>
            <?php 
			if( !empty($all_college_bus) ) {
			foreach ($all_college_bus as $college_bus) : 
			?>
            <tr id="row_<?=$i?>">
				<td><?=$i?></td>
                <td><?php echo $college_bus->reg_for_service_id; ?></td>
                <td><?php echo $college_bus->registration_no; ?></td>
                <td><?php 
				$reg_info= $this->registration_model->get_all_registration_info_by_id($college_bus->registration_no); 
				echo "<a href=".base_url().'registration/registration/view_single_registration/'.$college_bus->registration_no.">".$reg_info->first_name." ".$reg_info->middle_name." ".$reg_info->last_name."</a>";
				
				?>
</td>
                <td><?php if($college_bus->services_point_id) echo $this->ref_site_model->get_site_name_by_sp_id($college_bus->services_point_id); ?></td>
                <td><?php if($college_bus->services_point_id) echo $this->ref_site_model->get_site_name_by_id($college_bus->services_point_id); ?></td>
                <td><?php if($college_bus->services_package_id) echo $this->ref_services_model->get_package_name_by_id($college_bus->services_package_id); ?></td>
                <td><?php echo $college_bus->services_date; ?></td>
                <td>
				<?php 
				if($college_bus->services_status==0)
				{
				echo '<span class="label label-warning">Pending</span>';				
				}
			else if	($college_bus->services_status==1)
				{
				echo '<span class="label label-info">Process</span>';
				}
			else if	($college_bus->services_status==2)
				{
				echo '<span class="label label-success">Taken</span>';
				}
			else if	($college_bus->services_status==3)
				{
				echo '<span class="label label-important">Cancel</span>';
				}
			else if	($college_bus->services_status==4)
				{
                ...
                <?php //if ($this->authorization->is_permitted('add_college_bus')) : ?> 
                <td>
                <?php 
				if($college_bus->services_status==2)
				{
				if ($this->authorization->is_permitted('view_college_bus_services')) : 		
				?>                
                <a href="<?php echo base_url().'college_bus/college_bus/view_single_college_bus/'.$college_bus->reg_for_service_id;?>" class="btn btn-small btn-success"><?=lang('website_view')?></a>
                <?php
				endif; 
				if ($this->authorization->is_permitted('edit_college_bus_services')) : 	
				?>
                <a href="<?php echo base_url().'college_bus/college_bus/edit_single_college_bus/'.$college_bus->reg_for_service_id;?>" class="btn btn-small btn-warning"><?=lang('website_edit')?></a>
                <?php
				endif; 
				}
				else
				...
				?>
                <a href="<?php echo base_url().'college_bus/college_bus/add_college_bus/'.$college_bus->reg_for_service_id;?>" class="btn btn-small btn-info"><?=lang('website_add')?></a>
                <?php
				endif; 
				}
				?>

...
<?php echo $this->load->view('header'); ?>
    ...
    <?php echo form_open('blood_grouping/blood_grouping/search_blood_grouping_list') ?>
...
<th><?=lang('site')?></th>
                ...
                <th><?=lang('status')?></th>               
                <?php if ($this->authorization->is_permitted('add_blood_grouping')) : ?> 
			...
			?>
            <?php 
			if( !empty($all_blood_grouping) ) {
			foreach ($all_blood_grouping as $blood_grouping) : 
			?>
            <tr id="row_<?=$i?>">
				<td><?=$i?></td>
                <td><?php echo $blood_grouping->reg_for_service_id; ?></td>
                <td><?php echo $blood_grouping->registration_no; ?></td>
                <td><?php 
				$reg_info= $this->registration_model->get_all_registration_info_by_id($blood_grouping->registration_no); 
				echo "<a href=".base_url().'registration/registration/view_single_registration/'.$blood_grouping->registration_no.">".$reg_info->first_name." ".$reg_info->middle_name." ".$reg_info->last_name."</a>";
				
				?>
</td>
                <td><?php if($blood_grouping->services_point_id) echo $this->ref_site_model->get_site_name_by_sp_id($blood_grouping->services_point_id); ?></td>
                <td><?php if($blood_grouping->services_point_id) echo $this->ref_site_model->get_site_name_by_id($blood_grouping->services_point_id); ?></td>
                <td><?php if($blood_grouping->services_package_id) echo $this->ref_services_model->get_package_name_by_id($blood_grouping->services_package_id); ?></td>
                <td><?php echo $blood_grouping->services_date; ?></td>
                <td>
				<?php 
				if($blood_grouping->services_status==0)
				{
				echo '<span class="label label-warning">Pending</span>';				
				}
			else if	($blood_grouping->services_status==1)
				{
				echo '<span class="label label-info">Process</span>';
				}
			else if	($blood_grouping->services_status==2)
				{
				echo '<span class="label label-success">Taken</span>';
				}
			else if	($blood_grouping->services_status==3)
				{
				echo '<span class="label label-important">Cancel</span>';
				}
			else if	($blood_grouping->services_status==4)
				{
                ...
                <td>
                <?php 
				if($blood_grouping->services_status==2)
				{
				if ($this->authorization->is_permitted('view_blood_grouping')) : 		
				?>                
                <a href="<?php echo base_url().'blood_grouping/blood_grouping/view_single_blood_grouping/'.$blood_grouping->reg_for_service_id;?>" class="btn btn-small btn-success"><?=lang('website_view')?></a>
                <?php
				endif; 
				if ($this->authorization->is_permitted('edit_blood_grouping')) : 	
				?>
                <a href="<?php echo base_url().'blood_grouping/blood_grouping/edit_single_blood_grouping/'.$blood_grouping->reg_for_service_id;?>" class="btn btn-small btn-warning"><?=lang('website_edit')?></a>
                <?php
				endif; 
				}
				else
				...
				?>
                <a href="<?php echo base_url().'blood_grouping/blood_grouping/add_blood_grouping/'.$blood_grouping->reg_for_service_id;?>" class="btn btn-small btn-info"><?=lang('website_add')?></a>
                <?php
				endif; 
				}
				?>

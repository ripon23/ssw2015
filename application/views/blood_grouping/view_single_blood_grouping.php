...
<?php echo $this->load->view('header'); ?>
    ...
    <table class="table table-bordered">
    	<tr>
           <td>Blood Group</td>
           <td><?=$blood_grouping_info->blood_group?></td>
        </tr>       
    </table>           
    </div>            
    ...
    <table class="table table-striped">    	       
        <tr>
        	<td>Blood Group API status: </td>
            <td><?php 
			if($blood_grouping_info->upload_status==0)
			echo '<span class="label label-warning">Not Sent</span>';
			else if($blood_grouping_info->upload_status==1)
			echo '<span class="label label-success">Success</span>';			
			else if($blood_grouping_info->upload_status==2)
        ...
        <tr>
        	<td>Last Update: </td>
            <td><?=$blood_grouping_info->last_edit_date?> </td>
        </tr>
        <tr>
        	<td>Entry/Update user: </td>
            <td><?=$blood_grouping_info->edit_user_id?> </td>
        </tr>
        </table> 
        ...
        {
        ?>
        <a href="<?php echo base_url().'blood_grouping/blood_grouping/edit_single_blood_grouping/'.$blood_grouping_info->reg_for_service_id ;?>" class="btn btn-block btn-large btn-warning"><?=lang('website_edit')?></a>
        <?php
        }
        ?>
    </div><!-- /end span3 -->
